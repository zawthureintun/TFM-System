...
const PrintableHistory = React.forwardRef((props, ref) => {
  ...
  if (!statementData) {
    return <div ref={ref}><Typography>No data available to print.</Typography></div>;
  }
  else{
    console.log('Statment data is',statementData);
  }
    ...
    <div ref={ref}>
      <Container sx={{ p: 4, '@media print': { p: 2 } }}>
       ...
       <Box sx={{ p: 2, border: '1px solid #ddd', borderRadius: 1,width:'40%' }} mb={2}>
          <Typography variant="h6">{statementData.customer}</Typography>
          <Typography>
            Period: {format(new Date(statementData.startDate), 'dd-MM-yyyy')}-{format(new Date(statementData.endDate), 'dd-MM-yyyy')}
          </Typography>
        </Box>

        {/* Orders Table */}
        <Typography variant="h6" gutterBottom>Orders History</Typography>
            ...
            <TableBody>
              {statementData.orders.map((order) => (
                <TableRow key={order.id}>
                  ...
                  <TableCell align="right">{(order.amount - order.paidAmount).toLocaleString()}</TableCell>
                  <TableCell>{order.status}</TableCell>
                </TableRow>
              ))}
              <TableRow>
                <TableCell><strong>Total</strong></TableCell>
                <TableCell align="right"><strong>{statementData.totals.totalOrders.toLocaleString()}</strong></TableCell>
